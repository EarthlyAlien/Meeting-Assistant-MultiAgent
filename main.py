...
def main():
    ...
    parser.add_argument(
        "--audio", type=str, required=True, help="Path to the meeting audio file"
    )
    parser.add_argument(
        "--openai_api_key",
        type=str,
        help="OpenAI API key for summarization and extraction",
    )
    parser.add_argument(
        "--azure_speech_key", type=str, help="Azure Speech API key for transcription"
    )
    parser.add_argument(
        "--output",
        type=str,
        default="meeting_results.json",
        help="Output JSON file path",
    )
    parser.add_argument(
        "--report",
        type=str,
        default="meeting_report.md",
    ...
    print(
        f"- Transcription: {len(results['transcription'].get('transcription', ''))} characters"
    )
    print(f"- Summary: {len(results['summary'].get('summary', ''))} characters")
    print(
        f"- Action items: {len(results['action_items'].get('action_items', []))} items"
    )
